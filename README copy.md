...
You can also pass an LTL formula string directly and let LCRL convert it to an LDBA via [OWL](https://owl.model.in.tum.de/):

```python
>>> from lcrl.train import train
>>> from lcrl.environments.minecraft import minecraft
>>> task = train(minecraft, 'F (wood & X F tool_shed)',
...              algorithm='ql',
...              episode_num=500,
...              iteration_num_max=4000,
...              discount_factor=0.95,
...
For a complete script, see `./scripts/training_ql_ltl.py`.
...
LCRL now supports this conversion through [OWL](https://owl.model.in.tum.de/) by passing an LTL string directly to `train()`.
You can also build the automaton yourself (e.g. with OWL online [here](https://owl.model.in.tum.de/try/)) and pass an `LDBA` object manually.
